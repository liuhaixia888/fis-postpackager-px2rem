...
module.exports = function(ret, conf, settings, opt) {
    ...
    if(lib.isEmptyObject(settings)){
        
        for(key in ret.pkg){
            if(key.indexOf('.css') > -1){
                if(!map[key]){
                    ret.pkg[key]._content = ct._am2px(ret.pkg[key]._content);
                    map[key] = true;
                }
            }
        }
    ...
    //如果有配置, 找到打包后的所有css文件, 根据配置参数进行单位转换并重新写入文件
    fis.util.map(settings, function(file, opt){
        var defaults = {
            constant: 100,
            convertTo: 'rem',
            rounding: 'round',
            ...
            if(key.indexOf(file) > -1){
                if(!map[key]){
                    config.convertTo == 'rem' ?
                        ret.pkg[key]._content = ct._px2rem(ret.pkg[key]._content):
                        ret.pkg[key]._content = ct._rem2px(ret.pkg[key]._content);
                    map[key] = true;
                }            	
